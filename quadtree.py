...
that the rectangle can fully fit inside.'''
    ...
    def contains(self, rect):
        ...
        return (rect.x >= self.x) and (rect.y >= self.y) and (rect.x + rect.w < self.x + self.w) and (
...
up and tells that node to add it, else it adds the rectangle to this node.'''
        ...
        if self.level != 0 and len(self.nodes) == 0:
            ...
            # Lower right
            self.nodes.append(
                _QuadTreeNode(self.level - 1, self.x + self.w / 2, self.y + self.h / 2, self.w / 2, self.h / 2))
